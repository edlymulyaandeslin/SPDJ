    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            @if ($toggleAdd)
                <div class="mb-4">
                    @if (!$formEdit)
                        @livewire('rule.create-rule')
                    @else
                        @livewire('rule.update-rule', [$idFormEdit])
                    @endif
                </div>
            @endif
                ...
                <div class="relative px-4 py-5 shadow-md sm:rounded-lg sm:p-6">
                    <div class="flex flex-wrap items-center justify-between pb-4 space-y-4 flex-column sm:flex-row">
                        <div class="w-full text-end">
                            <x-button positive label="Rule Baru" right-icon="plus" wire:click="$toggle('toggleAdd')" />
                        </div>
                    ...
                    <table class="w-full text-sm text-left text-gray-500 rtl:text-right dark:text-gray-400">
                        ...
                        <tbody>
                                    ...
                                    <td class="px-6 py-4">
                                        <x-wui-dropdown>
                                            <x-dropdown.item label="Edit"
                                                wire:click="openFormEdit({{ $rule->id }})" />
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="py-4 text-xl text-center">
                                        No Data
                                    </td>
                                </tr>
                            @endforelse
